...
APP_NAME = os.environ.get("PWA_APP_NAME", "Fitness Gym Mixet").strip() or "Fitness Gym Mixet"
SHORT_NAME = os.environ.get("PWA_SHORT_NAME", "Fitness Gym").strip() or "Fitness Gym"
THEME_COLOR = os.environ.get("PWA_THEME_COLOR", "#0f4c81").strip() or "#0f4c81"
BG_COLOR = os.environ.get("PWA_BG_COLOR", "#06111f").strip() or "#06111f"
PWA_ICON_FILE = os.environ.get("PWA_ICON_FILE", "").strip()
ICON_TEXT = os.environ.get("PWA_ICON_TEXT", "FGM").strip() or "FGM"
...
FITNESS_DATA_FILE = os.path.join(DATA_ROOT, "fitness_mixet.json")

FITNESS_ACTIVITY_LEVELS = [
    {"key": "sedentary", "label": "Sedentario", "factor": 1.20, "description": "Poco movimento e allenamenti rari."},
    ...
    {"key": "very_active", "label": "Molto attivo", "factor": 1.90, "description": "Lavoro fisico o doppie sessioni."},
]
...
app.config["SESSION_COOKIE_NAME"] = "fitness_gym_mixet_session"
...
def round0(value):
    return round(float(value or 0.0), 0)
...
def fitness_activity_level(key):
    key = str(key or "").strip().lower()
    for item in FITNESS_ACTIVITY_LEVELS:
        if item["key"] == key:
            return item
...
def fitness_diet_goal(key):
    ...
    for item in FITNESS_DIET_GOALS:
        if item["key"] == key:
            return item
    return FITNESS_DIET_GOALS[0]


def fitness_default_state():
    return {
        "profiles": {},
        "plans": {},
        "checkins": {},
...
def fitness_load_state():
    if not os.path.exists(FITNESS_DATA_FILE):
        return fitness_default_state()
    with open(FITNESS_DATA_FILE, "r", encoding="utf-8") as handle:
        data = json.load(handle)
    state = fitness_default_state()
    state.update(data or {})
    state["profiles"] = state.get("profiles", {}) or {}
    state["plans"] = state.get("plans", {}) or {}
    state["checkins"] = state.get("checkins", {}) or {}
    return state


def fitness_save_state(state):
    os.makedirs(os.path.dirname(FITNESS_DATA_FILE), exist_ok=True)
    with open(FITNESS_DATA_FILE, "w", encoding="utf-8") as handle:
        json.dump(state, handle, ensure_ascii=False, indent=2)


def fitness_bmi(weight_kg, height_cm):
...
def fitness_bmr(sex, weight_kg, height_cm, age_years):
...
def fitness_body_fat_deurenberg(sex, weight_kg, height_cm, age_years):
    sex_flag = 1 if str(sex).strip().lower() == "male" else 0
    bmi_value = fitness_bmi(weight_kg, height_cm)
    return 1.20 * bmi_value + 0.23 * float(age_years) - 10.8 * sex_flag - 5.4


def fitness_body_fat_navy(sex, height_cm, waist_cm, neck_cm, hips_cm=None):
    sex = str(sex or "").strip().lower()
    ...
    if sex == "male":
        ...
        if diff <= 0:
            raise ValueError("Le circonferenze non sono coerenti per il metodo Navy.")
        return 86.010 * math.log10(diff) - 70.041 * math.log10(height_in) + 36.76
    if sex == "female":
        hip_in = float(hips_cm) / 2.54
        diff = waist_in + hip_in - neck_in
        if diff <= 0:
            raise ValueError("Le circonferenze non sono coerenti per il metodo Navy.")
        return 163.205 * math.log10(diff) - 97.684 * math.log10(height_in) - 78.387
    raise ValueError("Sesso non valido.")


def fitness_body_fat_estimate(payload):
    sex = payload["sex"]
    method = str(payload.get("bf_method") or "auto").strip().lower()
    manual = to_float(payload.get("body_fat_manual"), None)
    waist_cm = to_float(payload.get("waist_cm"), None)
    neck_cm = to_float(payload.get("neck_cm"), None)
    hips_cm = to_float(payload.get("hips_cm"), None)
    can_navy = sex in {"male", "female"} and waist_cm is not None and neck_cm is not None and (sex == "male" or hips_cm is not None)

    if method == "manual":
        if manual is None:
            raise ValueError("Inserisci la BF manuale.")
        return {
            "percent": clamp(float(manual), 3.0, 80.0),
            "method": "manuale",
            "label": "BF manuale",
            "details": "Valore inserito manualmente.",
        }

    if method == "navy" or (method == "auto" and can_navy):
        if not can_navy:
            if method == "navy":
                raise ValueError("Per il metodo Navy servono vita, collo e fianchi se sei donna.")
        else:
            navy = fitness_body_fat_navy(sex, payload["height_cm"], waist_cm, neck_cm, hips_cm)
            return {
                "percent": clamp(navy, 3.0, 80.0),
                "method": "navy",
                "label": "Metodo Navy",
                "details": "Stima da circonferenze.",
            }

    estimate = fitness_body_fat_deurenberg(sex, payload["weight_kg"], payload["height_cm"], payload["age_years"])
    return {
        "percent": clamp(estimate, 3.0, 80.0),
        "method": "estimate",
        "label": "Stima BMI",
        "details": "Stima automatica basata su BMI, età e sesso.",
    }


def fitness_calculate_plan(payload):
    sex = str(payload.get("sex") or "").strip().lower()
    if sex not in {"male", "female"}:
        raise ValueError("Seleziona sesso maschile o femminile.")

    age_years = clamp(to_int(payload.get("age_years"), None) or 0, 14, 99)
    height_cm = clamp(to_float(payload.get("height_cm"), None) or 0.0, 120.0, 230.0)
    weight_kg = clamp(to_float(payload.get("weight_kg"), None) or 0.0, 30.0, 400.0)
    diet_goal = fitness_diet_goal(payload.get("diet_goal"))
    ideal_weight_kg = to_float(payload.get("ideal_weight_kg"), None)
    if ideal_weight_kg is not None:
        ideal_weight_kg = clamp(ideal_weight_kg, 30.0, 400.0)
    target_bf = to_float(payload.get("target_body_fat_percent"), None)
    if target_bf is not None:
        target_bf = clamp(target_bf, 3.0, 60.0)
    ...
    bf_info = fitness_body_fat_estimate(
        {
            ...
            "height_cm": height_cm,
            "age_years": age_years,
            "bf_method": payload.get("bf_method", "auto"),
            "body_fat_manual": payload.get("body_fat_manual"),
            "waist_cm": payload.get("waist_cm"),
            "neck_cm": payload.get("neck_cm"),
            "hips_cm": payload.get("hips_cm"),
        }
    )

    body_fat_percent = bf_info["percent"]
    bmi_value = fitness_bmi(weight_kg, height_cm)
    bmr_value = fitness_bmr(sex, weight_kg, height_cm, age_years)
    tdee_value = bmr_value * activity_factor

    lean_mass_kg = weight_kg * (1.0 - body_fat_percent / 100.0)
    fat_mass_kg = weight_kg - lean_mass_kg
    target_weight_from_bf_kg = None
    if target_bf is not None:
        target_weight_from_bf_kg = lean_mass_kg / (1.0 - target_bf / 100.0)
    display_target_weight_kg = ideal_weight_kg if ideal_weight_kg is not None else target_weight_from_bf_kg
    weight_delta_kg = (display_target_weight_kg - weight_kg) if display_target_weight_kg is not None else 0.0
    calorie_target = max(0.0, tdee_value * (1.0 + (diet_goal["adjustment_percent"] / 100.0)))
    daily_delta = calorie_target - tdee_value
    direction = diet_goal["direction"]
    ...
    warnings = []
    if abs(diet_goal["adjustment_percent"]) >= 20:
        warnings.append("Il piano e aggressivo: monitora energia e recupero.")
    min_floor = 1500.0 if sex == "male" else 1200.0
    if calorie_target < min_floor:
        warnings.append("Le calorie stimate scendono molto in basso: valuta piu tempo o un target BF meno spinto.")
    if weekly_change_pct > 1.25:
        warnings.append("La velocita settimanale stimata e alta.")
    if bf_info["method"] == "estimate":
        warnings.append("La BF e stimata dal BMI: se puoi, inserisci le circonferenze o un valore manuale.")

    summary_parts = [f"Obiettivo: {diet_goal['label']} a {calorie_target:.0f} kcal/giorno."]
    if display_target_weight_kg is not None:
        summary_parts.append(f"Peso ideale {display_target_weight_kg:.1f} kg.")
    if target_bf is not None:
        summary_parts.append(f"Target BF {target_bf:.1f}%.")
    ...
    return {
        ...
        "inputs": {
            ...
            "activity_key": activity_item["key"],
            "activity_label": activity_item["label"],
            "activity_factor": round(activity_factor, 3),
            "diet_goal": diet_goal["key"],
            "diet_goal_label": diet_goal["label"],
            "diet_goal_adjustment_percent": diet_goal["adjustment_percent"],
            "bf_method": payload.get("bf_method", "auto"),
            "body_fat_manual": round1(to_float(payload.get("body_fat_manual"), 0.0) or 0.0) if to_float(payload.get("body_fat_manual"), None) is not None else None,
            "waist_cm": round1(to_float(payload.get("waist_cm"), 0.0) or 0.0) if to_float(payload.get("waist_cm"), None) is not None else None,
            "neck_cm": round1(to_float(payload.get("neck_cm"), 0.0) or 0.0) if to_float(payload.get("neck_cm"), None) is not None else None,
        ...
        "metrics": {
            ...
            "body_fat_method": bf_info["method"],
            "body_fat_label": bf_info["label"],
            "body_fat_details": bf_info["details"],
            "lean_mass_kg": round1(lean_mass_kg),
            "fat_mass_kg": round1(fat_mass_kg),
            "ideal_weight_kg": round1(display_target_weight_kg) if display_target_weight_kg is not None else None,
            "target_weight_from_bf_kg": round1(target_weight_from_bf_kg) if target_weight_from_bf_kg is not None else None,
            "target_body_fat_percent": round1(target_bf) if target_bf is not None else None,
            "diet_goal": diet_goal["key"],
            "diet_goal_label": diet_goal["label"],
            "diet_goal_adjustment_percent": diet_goal["adjustment_percent"],
            "weight_delta_kg": round1(weight_delta_kg),
            "weekly_change_kg": round1(weekly_change_kg),
            "weekly_change_percent": round1(weekly_change_pct),
            "daily_calorie_delta": round0(daily_delta),
            "calorie_target": round0(calorie_target),
            "direction": direction,
        },
        "macros": {
            ...
            "fat_g": round1(fat_g),
            "fat_kcal": round0(fat_kcal),
            "carbs_g": round1(carbs_g),
            "carbs_kcal": round0(max(0.0, carbs_kcal)),
        },
        "warnings": warnings,
...
def ensure_fitness_schema():
    ...
    with store.db_connect() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                """
                CREATE INDEX IF NOT EXISTS idx_fitness_plans_user_created
                ON fitness_plans (user_email, created_at DESC)
                """
            )
            cur.execute(
                """
            ...
            cur.execute(
                """
                CREATE INDEX IF NOT EXISTS idx_fitness_checkins_user_date
                ON fitness_checkins (user_email, measured_on DESC, created_at DESC)
                """
            )
...
def manifest_webmanifest():
    ...
    manifest = {
        ...
        "description": "Calcola BMI, BMR, TDEE, BF e un piano dieta con obiettivi opzionali in una PWA mobile.",
        "start_url": "/",
        "scope": "/",
        "display": "standalone",
        "orientation": "portrait",
        "background_color": pwa_color(BG_COLOR, "#06111f"),
        "theme_color": pwa_color(THEME_COLOR, "#0f4c81"),
        "icons": [
            {
                "src": icon_src,
                "sizes": icon["sizes"],
...
def service_worker():
    ...
    script = """
const CACHE_NAME = "fitness-gym-mixet-__ASSET_VERSION__";
const APP_SHELL = [
  "/",
...
def fitness_get_profile(email):
    ...
    state = fitness_load_state()
    return state.get("profiles", {}).get(email, {}) or {}


def fitness_save_profile(email, profile):
    email = normalize_email(email)
    payload = json_safe(profile or {})
    payload["email"] = email
    ...
    if DB_MODE:
        with store.db_connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO fitness_profiles (user_email, profile_json, updated_at)
                    VALUES (%s, %s::jsonb, NOW())
                    ON CONFLICT (user_email) DO UPDATE SET
    ...
    state = fitness_load_state()
    state.setdefault("profiles", {})[email] = payload
    fitness_save_state(state)
    return payload


def fitness_list_plans(email, limit=8):
    email = normalize_email(email)
    if DB_MODE:
        with store.db_connect() as conn:
            with conn.cursor() as cur:
    ...
    state = fitness_load_state()
    return (state.get("plans", {}).get(email, []) or [])[:limit]


def fitness_get_latest_plan(email):
    plans = fitness_list_plans(email, limit=1)
    return plans[0] if plans else None


def fitness_save_plan(email, plan):
    email = normalize_email(email)
    payload = json_safe(plan or {})
    ...
    if DB_MODE:
        with store.db_connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO fitness_plans (plan_id, user_email, plan_json, created_at)
                    VALUES (%s, %s, %s::jsonb, NOW())
                    ON CONFLICT (plan_id) DO UPDATE SET
                        plan_json = EXCLUDED.plan_json
                    """,
                    (plan_id, email, json.dumps(payload, ensure_ascii=False)),
                )
        return payload

    state = fitness_load_state()
    plans = state.setdefault("plans", {}).setdefault(email, [])
    plans.insert(0, payload)
    state["plans"][email] = plans[:20]
    fitness_save_state(state)
    return payload


def fitness_list_checkins(email, limit=10):
    email = normalize_email(email)
    if DB_MODE:
        with store.db_connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    SELECT checkin_id, measured_on, weight_kg, body_fat_percent, notes, created_at
        ...
        for row in rows:
            items.append(
                {
                    "checkin_id": row[0],
                    "measured_on": row[1].isoformat() if row[1] else "",
                    "weight_kg": round1(row[2]),
                    "body_fat_percent": round1(row[3]) if row[3] is not None else None,
                    "notes": row[4] or "",
    ...
    state = fitness_load_state()
    return (state.get("checkins", {}).get(email, []) or [])[:limit]


def fitness_save_checkin(email, item):
    email = normalize_email(email)
    payload = json_safe(item or {})
    payload["checkin_id"] = str(payload.get("checkin_id") or uuid.uuid4().hex)
    payload["user_email"] = email
    payload["created_at"] = payload.get("created_at") or iso_now()
    if DB_MODE:
        with store.db_connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO fitness_checkins (checkin_id, user_email, measured_on, weight_kg, body_fat_percent, notes, created_at)
                    ...
                    (
                        payload["checkin_id"],
                        email,
                        parse_date(payload.get("measured_on"), date.today()) or date.today(),
    ...
    state = fitness_load_state()
    items = state.setdefault("checkins", {}).setdefault(email, [])
    items.insert(0, payload)
    state["checkins"][email] = items[:50]
    fitness_save_state(state)
    return payload


def fitness_get_state(email):
    return {
        "storage_mode": "database" if DB_MODE else "file",
        "profile": fitness_get_profile(email),
        "latest_plan": fitness_get_latest_plan(email),
        "plans": fitness_list_plans(email, limit=8),
...
def pwa_icon():
    ...
    start = escape(pwa_color(THEME_COLOR, "#0f4c81"))
    accent = "#27e89d"
    glow = "#00d1ff"
    bg = escape(pwa_color(BG_COLOR, "#06111f"))
...
<circle cx="406" cy="114" r="78" fill="{glow}" opacity="0.14"/>
<circle cx="124" cy="396" r="92" fill="{accent}" opacity="0.08"/>
<rect x="94" y="224" width="324" height="64" rx="32" fill="url(#mix)"/>
<rect x="48" y="196" width="46" height="120" rx="20" fill="url(#mix)"/>
<rect x="418" y="196" width="46" height="120" rx="20" fill="url(#mix)"/>
<rect x="142" y="176" width="24" height="160" rx="12" fill="white" opacity="0.88"/>
<rect x="346" y="176" width="24" height="160" rx="12" fill="white" opacity="0.88"/>
<path d="M172 372c24 22 52 34 84 34s60-12 84-34" fill="none" stroke="white" stroke-width="18" stroke-linecap="round" opacity="0.82"/>
<text x="256" y="202" text-anchor="middle" font-family="Georgia, 'Times New Roman', serif" font-size="138" font-weight="700" fill="white">{label}</text>
</svg>"""
...
def api_state():
    user = g.current_user
    return jsonify({"ok": True, **fitness_get_state(user["email"]), "user": user})


@app.post("/api/plan/calc")
@login_required
def api_plan_calc():
    user = g.current_user
    payload = request.get_json(silent=True) or {}
    try:
        plan = fitness_calculate_plan(payload)
    except ValueError as exc:
        return jsonify({"ok": False, "message": str(exc)}), 400

    profile = {
        "sex": plan["inputs"]["sex"],
        "age_years": plan["inputs"]["age_years"],
        "height_cm": plan["inputs"]["height_cm"],
        "weight_kg": plan["inputs"]["weight_kg"],
        "activity_key": plan["inputs"]["activity_key"],
        "activity_factor": plan["inputs"]["activity_factor"],
        "diet_goal": plan["inputs"]["diet_goal"],
        "diet_goal_label": plan["inputs"]["diet_goal_label"],
        "diet_goal_adjustment_percent": plan["inputs"]["diet_goal_adjustment_percent"],
        "bf_method": plan["inputs"]["bf_method"],
        "body_fat_manual": plan["inputs"]["body_fat_manual"],
        "waist_cm": plan["inputs"]["waist_cm"],
        "neck_cm": plan["inputs"]["neck_cm"],
    ...
    with lock:
        fitness_save_profile(user["email"], profile)
        stored_plan = fitness_save_plan(user["email"], plan)
    return jsonify(
        {
            "ok": True,
            "message": "Piano salvato.",
            "profile": profile,
            "plan": stored_plan,
            "latest_plan": stored_plan,
            "plans": fitness_list_plans(user["email"], limit=8),
            "checkins": fitness_list_checkins(user["email"], limit=10),
        }
    )
...
def api_checkins():
    user = g.current_user
    payload = request.get_json(silent=True) or {}
    weight_kg = to_float(payload.get("weight_kg"), None)
    if weight_kg is None or weight_kg <= 0:
        return jsonify({"ok": False, "message": "Peso non valido."}), 400
    item = {
        "measured_on": (parse_date(payload.get("measured_on"), date.today()) or date.today()).isoformat(),
        "weight_kg": round1(weight_kg),
        "body_fat_percent": round1(to_float(payload.get("body_fat_percent"), None)) if to_float(payload.get("body_fat_percent"), None) is not None else None,
        "notes": clean_text(payload.get("notes"), "", 180),
    }
    with lock:
        saved = fitness_save_checkin(user["email"], item)
    return jsonify({"ok": True, "message": "Check-in salvato.", "checkin": saved, "checkins": fitness_list_checkins(user["email"], limit=10)})
...
def api_export_csv():
    user = g.current_user
    plans = fitness_list_plans(user["email"], limit=200)
    output = io.StringIO()
    ...
    for plan in plans:
        ...
        writer.writerow(
            [
                plan.get("created_at", ""),
                inputs.get("sex", ""),
                inputs.get("weight_kg", ""),
                metrics.get("body_fat_percent", ""),
                metrics.get("bmi", ""),
                metrics.get("bmr", ""),
                metrics.get("tdee", ""),
                inputs.get("diet_goal_label", inputs.get("diet_goal", "")),
                inputs.get("ideal_weight_kg", ""),
                inputs.get("target_body_fat_percent", ""),
                metrics.get("calorie_target", ""),
                metrics.get("daily_calorie_delta", ""),
    ...
    return send_file(
        ...
        mimetype="text/csv",
        as_attachment=True,
        download_name=f"fitness_gym_mixet_{user['email'].split('@')[0]}.csv",
    )
